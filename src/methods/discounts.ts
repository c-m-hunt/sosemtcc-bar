import { Client, CatalogObject, CatalogObjectBatch, BatchUpsertCatalogObjectsRequest } from 'square'
...
export const getClubRateDiscount = async (client: Client) => {
    ...
    const discount = items.result.objects?.filter(item => item.discountData?.name === clubDiscountName);
    if (!discount || discount.length === 0) {
      throw Error("No club discount found");
    }
    const pricingRule = items.result.objects?.filter(item => {
        return item.type === 'PRICING_RULE' && item.pricingRuleData?.discountId === discount[0].id;
    });
    if (!pricingRule || pricingRule.length === 0) {
      throw Error("No pricing rule found");
    }
    const productSet = items.result.objects?.filter(item => {
      return item.type === 'PRODUCT_SET' && item.id === pricingRule[0].pricingRuleData?.matchProductsId
    });
...
export const deleteClubRateDiscount = async (client: Client) => {
    ...
    return await deleteInventoryItems(client, existingDiscount.map(item => item.id));
}
...
export const insertClubRateDiscount = async (client: Client) => {
    ...
    const pricingRule: CatalogObject = {
        ...
        isDeleted: false,
        presentAtAllLocations: true,
        pricingRuleData: { discountId: discount.id, matchProductsId: productSet.id }
      }
    ...
    const response = await discountsApi.batchUpsertCatalogObjects(batchUpsertRequest);
